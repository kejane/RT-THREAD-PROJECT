...
extern uint8_t reflective;
extern uint8_t background;
extern uint8_t *datazoom;
//extern rt_uint8_t lockFlag;
rt_uint8_t lockHelper=0;//we need lockHelper because the pin can't be high for a long time, or the device may be burned
uint8_t data = 0;
void my_irq_handle(int args)
{
    static uint8_t cnt=0;
    datazoom[cnt++] = args;
    datazoom[cnt++] = person;
    datazoom[cnt++] = reflective;
    datazoom[cnt++] = background;
    data++;
}
void lock_handle(void)
{
    //lockFlag = 0;
    //lockHelper = 0;
    rt_kprintf("llllllllllllllllllllll\n");
}
...
void key_init()
{
    ...
    ret = rt_pin_attach_irq(pin_number, PIN_IRQ_MODE_FALLING , my_irq_handle, 0);
    ret = rt_pin_irq_enable(pin_number, PIN_IRQ_ENABLE);
    pin_number = 17;
    rt_pin_mode(pin_number, PIN_MODE_INPUT_PULLUP);
    rt_pin_write(pin_number, PIN_HIGH);
    ret = rt_pin_attach_irq(pin_number, PIN_IRQ_MODE_FALLING , my_irq_handle, 1);
    ret = rt_pin_irq_enable(pin_number, PIN_IRQ_ENABLE);
    pin_number = 18;
...
extern rt_uint8_t assureFlag;
void unlock()
{
    /*unlock and change the stateFlag*/
    if(lockHelper<=1)
    {
        rt_pin_write(18, PIN_HIGH);
        //lockFlag = 1;//made by procession
        lockHelper++ ;
    }
    /* just get the pin low*/
    else if(lockHelper==2){
