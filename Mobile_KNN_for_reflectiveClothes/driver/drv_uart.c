...
rt_device_t serial;
rt_uint8_t command_buffer[20];
extern rt_uint8_t fitMode;
void uart_input(rt_device_t dev, rt_size_t size)
{
    rt_memset(command_buffer, 0, 20);
    rt_device_read(serial, NULL, command_buffer,10);
//    rt_kprintf("%s",command_buffer);
    switch(command_buffer[0]){
    case 'F' :  fitMode = 1;rt_kprintf("get1\n");break;
    case 'R' :  fitMode = 0;rt_kprintf("get0\n");break;
    default  :  break;
    }
}
void myUart_init()
